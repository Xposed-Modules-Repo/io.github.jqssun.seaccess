...
Therefore, this module instead modifies the response there without any drastic changes in terms of how `AccessControlEnforcer` behaves. Of course, it is also possible to become even more specific with the patching, down to the exact application signature rather than using application ID, but this may be a bit more cumbersome to support and maintain.

For users that are still not entirely convinced with how secure this is, you are welcome to cut down the list even more and include your own application ID in your build. Since Android by default does not allow app signatures to be overwritten duing updates, you should be protected so long as your custom LPA app is signed properly 👍
